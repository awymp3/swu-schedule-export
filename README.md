...
- 首次运行需要联网下载浏览器驱动和 Python 依赖。若电脑没有可用的 Python 3，Windows 和 macOS 启动脚本会将运行时下载到项目目录的 `.runtime` 中，不会修改系统 PATH。
- 教务账号和密码只保存在项目目录的 `.env` 文件中，不会上传。该文件已被 Git 忽略，不应分享给其他人。
...
1. 第一次使用时输入教务账号和密码；勾选记住后，账号会写入 `.env`。
...
删除项目目录中的 `.env` 后重新运行，程序会再次要求输入账号和密码。

### 本地运行时或依赖下载失败

检查网络后重试。需要重新下载时，可删除项目目录中的 `.runtime`；账号信息不会受到影响。
